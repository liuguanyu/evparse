package com.greensock {
	...
	public class TweenLite extends TweenCore {
		...
		public function TweenLite(param1:Object, param2:Number, param3:Object) {
			...
			this._overwrite = !("overwrite" in param3) || !overwriteManager.enabled && param3.overwrite > 1?overwriteManager.mode:int(param3.overwrite);
			...
			if(param1 in masterList) {
				...
				while(_loc4_--) {
					if(!TweenLite(_loc3_[_loc4_]).gc) {
						if(param2) {
							TweenLite(_loc3_[_loc4_]).complete(false,false);
						} else {
							TweenLite(_loc3_[_loc4_]).setEnabled(false,false);
						}
					}
				}
