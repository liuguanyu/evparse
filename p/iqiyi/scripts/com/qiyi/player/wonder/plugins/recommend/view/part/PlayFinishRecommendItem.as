package com.qiyi.player.wonder.plugins.recommend.view.part {
	...
	public class PlayFinishRecommendItem extends Sprite {
		...
		private function initBigItem() : void {
			...
			var _loc1_:String = PlayFinishRecommendUtils.getUpdate(this._data.channel,this._data.timeLength,this._data.updateFlag,this._data.latestOrder);
			this._coverEpisodeTF = FastCreator.createLabel(_loc1_,16777215);
			this._coverEpisodeTF.x = this._cover.width - this._coverEpisodeTF.width;
			this._cover.bigCoverMC.addChild(this._coverEpisodeTF);
			var _loc2_:String = this._data.albumName == ""?this._data.videoName:this._data.albumName;
			this._coverTitleTF = FastCreator.createLabel("",16777215);
			this._coverTitleTF.text = _loc2_.length > 14?_loc2_.slice(0,13) + "...":_loc2_;
			this._coverTitleTF.x = this._cover.width / 2 - this._coverTitleTF.width / 2;
			this._cover.bigCoverMC.addChild(this._coverTitleTF);
			var _loc3_:String = this._data.focus.length > 14?this._data.focus.slice(0,13) + "...":this._data.focus;
			this._coverSubTitleTF = FastCreator.createLabel(_loc3_,16777215);
		...
		private function initSmallItem() : void {
			...
			addChild(this._cover);
			this.addFlow();
			var _loc1_:String = this._data.albumName == ""?this._data.videoName:this._data.albumName;
			this._coverTitleTF = FastCreator.createLabel(_loc1_.length > 11?_loc1_.slice(0,11) + "...":_loc1_,16777215);
			this._coverTitleTF.x = this._cover.width / 2 - this._coverTitleTF.width / 2;
		...
		private function addFlow() : void {
			...
			this._flowTitleTF.text = _loc1_.length > 21?_loc1_.slice(0,21) + "...":_loc1_;
		...
		private function loaderPic(param1:String) : void {
			var _loc2_:Number = this._isBig?284:RecommendDef.PLAY_FINISH_SMALL_ITEM_WIDTH;
			var _loc3_:Number = this._isBig?160:RecommendDef.PLAY_FINISH_SMALL_ITEM_HEIGHT;
			var param1:String = param1.replace(".jpg","_" + _loc2_ + "_" + _loc3_ + ".jpg");
			param1 = param1.replace(".JPG","_" + _loc2_ + "_" + _loc3_ + ".JPG");
		...
		private function onLoaderComplete(param1:Event) : void {
			var _loc2_:Bitmap = new Bitmap(this._picLoader.content["bitmapData"]);
			_loc2_.smoothing = true;
			_loc2_.width = this._isBig?RecommendDef.PLAY_FINISH_BIG_ITEM_WIDTH:RecommendDef.PLAY_FINISH_SMALL_ITEM_WIDTH;
			_loc2_.height = this._isBig?RecommendDef.PLAY_FINISH_BIG_ITEM_HEIGHT:RecommendDef.PLAY_FINISH_SMALL_ITEM_HEIGHT;
			this._imgContainer.addChild(_loc2_);
		...
		public function destroy() : void {
			...
			if((this._imgContainer) && (this._imgContainer.parent)) {
				while(this._imgContainer.numChildren > 0) {
					...
					this._imgContainer.removeChild(_loc1_);
					_loc1_ = null;
				}
			...
			if((this._flow) && (this._flow.parent)) {
				while(this._flow.numChildren > 0) {
					_loc2_ = this._flow.getChildAt(0);
					this._flow.removeChild(_loc2_);
					_loc2_ = null;
				}
			...
			if((this._cover) && (this._cover.parent)) {
				while(this._cover.numChildren > 0) {
					_loc3_ = this._cover.getChildAt(0);
					this._cover.removeChild(_loc3_);
					_loc3_ = null;
				}
