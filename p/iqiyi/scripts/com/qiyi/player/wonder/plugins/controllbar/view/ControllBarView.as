package com.qiyi.player.wonder.plugins.controllbar.view {
	...
	public class ControllBarView extends BasePanel {
			...
			switch(param1) {
				...
				case ControllBarDef.STATUS_LIST_BTN_SHOW:
					...
					if(this._likeListBtn) {
						this._likeListBtn.updateBtnText(_loc4_);
						this._likeListBtn.visible = true;
					} else {
						this._likeListBtn = new ControllBarButton(_loc4_);
					...
					ToolTip.getInstance().registerComponent(this._likeListBtn,_loc4_);
					this.onBtnsResize(GlobalStage.stage.stageWidth,GlobalStage.stage.stageHeight);
					break;
				case ControllBarDef.STATUS_TVLIST_BTN_SHOW:
					_loc5_ = "剧集列表";
					if(this._likeListBtn) {
						this._likeListBtn.updateBtnText(_loc5_);
						this._likeListBtn.visible = true;
					} else {
						this._likeListBtn = new ControllBarButton(_loc5_);
						this._controlBarUI.addChild(this._likeListBtn);
						this._likeListBtn.addEventListener(MouseEvent.CLICK,this.onTvListBtnClick);
					}
					ToolTip.getInstance().unregisterComponent(this._likeListBtn);
					ToolTip.getInstance().registerComponent(this._likeListBtn,_loc5_);
					this.onBtnsResize(GlobalStage.stage.stageWidth,GlobalStage.stage.stageHeight);
					break;
		...
		private function onBtnsResize(param1:int, param2:int) : void {
			...
			_loc3_ = this._controlBarUI.nextVideoBtn.x + this._controlBarUI.nextVideoBtn.width * int(this._controlBarUI.nextVideoBtn.visible);
			if(this._likeListBtn) {
				...
				_loc3_ = this._likeListBtn.x + this._likeListBtn.width * int(this._likeListBtn.visible);
			}
			if(this._timeShow) {
				this._timeShow.x = _loc3_;
				_loc3_ = this._timeShow.x + this._timeShow.width * int(this._timeShow.visible);
			}
			_loc3_ = 0;
			...
			var _loc4_:Boolean = this._status.hasStatus(ControllBarDef.STATUS_VOLUME_BAR_SHOW);
			this._volumeControlView.x = this._controlBarUI.unFoldBtn.x - this._volumeControlView.width * int(_loc4_) - 16;
			_loc3_ = this._volumeControlView.x;
			if(this._definitionBtn) {
				this._definitionBtn.x = this._volumeControlView.x - this._definitionBtn.width * int(this._definitionBtn.visible);
				_loc3_ = this._definitionBtn.x;
			}
			if(this._filterBtn) {
				this._filterBtn.x = _loc3_ - this._filterBtn.width * int(this._filterBtn.visible);
				_loc3_ = this._filterBtn.x;
			}
			if(this._trackBtn) {
				this._trackBtn.x = _loc3_ - this._trackBtn.width * int(this._trackBtn.visible);
				_loc3_ = this._trackBtn.x;
			}
			if(this._captionBtn) {
				this._captionBtn.x = _loc3_ - this._captionBtn.width * int(this._captionBtn.visible);
				_loc3_ = this._captionBtn.x;
			}
			if(this._d3Btn) {
				this._d3Btn.x = _loc3_ - this._d3Btn.width * int(this._d3Btn.visible);
				_loc3_ = this._d3Btn.x;
			}
		...
		private function updateDisplayTime(param1:int, param2:int) : void {
			...
			if(this._timeShow) {
				this._timeShow.updateTime(_loc3_,_loc4_);
			}
		}
		...
		private function onD3BtnClick(param1:MouseEvent) : void {
			...
			dispatchEvent(new ControllBarEvent(ControllBarEvent.Evt_D3BtnClick,_loc2_));
		}
