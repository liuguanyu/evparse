package com.qiyi.player.wonder.plugins.dock.view
{
	...
	public class DockViewMediator extends Mediator implements ISwitch
	{
		...
		override public function handleNotification(param1:INotification) : void
		{
			...
			switch(_loc3)
			{
				case DockDef.NOTIFIC_ADD_STATUS:
					this._dockView.onAddStatus(int(_loc2));
					break;
				case DockDef.NOTIFIC_REMOVE_STATUS:
					this._dockView.onRemoveStatus(int(_loc2));
					break;
				case BodyDef.NOTIFIC_RESIZE:
					this._dockView.onResize(_loc2.w,_loc2.h);
					break;
				case BodyDef.NOTIFIC_FULL_SCREEN:
					if((SwitchManager.getInstance().getStatus(SwitchDef.ID_SHOW_DOCK_LIGHT)) && !Boolean(_loc2))
					{
				...
				case BodyDef.NOTIFIC_JS_LIGHT_CHANGED:
					...
					PingBack.getInstance().userActionPing(PingBackDef.LIGHT,_loc5.curActor.currentTime);
					break;
		...
		private function checkShowStatus() : Boolean
		{
			...
			if((SwitchManager.getInstance().getStatus(SwitchDef.ID_SHOW_DOCK) && _loc1.curActor.hasStatus(BodyDef.PLAYER_STATUS_ALREADY_READY) && !_loc1.curActor.hasStatus(BodyDef.PLAYER_STATUS_STOPPING) && !_loc1.curActor.hasStatus(BodyDef.PLAYER_STATUS_STOPED) && !_loc2.hasStatus(ADDef.STATUS_LOADING) && !_loc2.hasStatus(ADDef.STATUS_PLAYING) && !_loc2.hasStatus(ADDef.STATUS_PAUSED) && _loc1.curActor.movieModel) && (!(_loc1.curActor.movieModel.screenType == ScreenEnum.THREE_D)) && !_loc1.curActor.smallWindowMode)
			{
				return true;
			}
			return false;
		...
		{
			var _loc1:PlayerProxy = facade.retrieveProxy(PlayerProxy.NAME) as PlayerProxy;
			if((SwitchManager.getInstance().getStatus(SwitchDef.ID_SHOW_DOCK_OFFLINE_WATCH)) && (_loc1.curActor.movieModel))
			{
				if(!_loc1.curActor.movieModel.member)
				{
					return true;
				}
			}
		...
		private function onCheckUserComplete() : void
		{
			...
			this._dockView.onUserInfoChanged(_loc2);
		}
			...
			switch(param1)
			{
				...
				case BodyDef.PLAYER_STATUS_ALREADY_INFO_READY:
					if(param2)
					{
						...
						if((_loc4.curActor.movieInfo) && (_loc4.curActor.movieInfo.allowDownload))
						{
		...
		private function onShareBtnClick(param1:MouseEvent) : void
		{
			...
			if(_loc2.hasStatus(ShareDef.STATUS_OPEN))
			{
		...
		private function onCloseLightBtnClick(param1:MouseEvent) : void
		{
			var _loc2:PlayerProxy = facade.retrieveProxy(PlayerProxy.NAME) as PlayerProxy;
			var _loc3:JavascriptAPIProxy = facade.retrieveProxy(JavascriptAPIProxy.NAME) as JavascriptAPIProxy;
			_loc3.callJsSetLight(false);
		}
		
		private function onOfflineWatchBtnClick(param1:MouseEvent) : void
		{
			var _loc3:OfflineWatchProxy = null;
			var _loc2:JavascriptAPIProxy = facade.retrieveProxy(JavascriptAPIProxy.NAME) as JavascriptAPIProxy;
			if(FlashVarConfig.owner == FlashVarConfig.OWNER_CLIENT || FlashVarConfig.owner == FlashVarConfig.OWNER_PAGE && (_loc2.checkClientInstall()))
			{
				_loc2.callJsDownload();
			}
			else
			{
				_loc3 = facade.retrieveProxy(OfflineWatchProxy.NAME) as OfflineWatchProxy;
				if(_loc3.hasStatus(OfflineWatchDef.STATUS_OPEN))
				{
