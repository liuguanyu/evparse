	...
	public class DockView extends BasePanel
	{
		...
		public function onAddStatus(param1:int) : void
		{
			...
			switch(param1)
			{
				...
				case DockDef.STATUS_SHOW:
					_loc2 = GlobalStage.stage.stageWidth - this._bg.width;
		...
		public function onRemoveStatus(param1:int) : void
		{
			...
			switch(param1)
			{
				case DockDef.STATUS_OPEN:
					...
					break;
				case DockDef.STATUS_SHOW:
					_loc2 = GlobalStage.stage.stageWidth;
					TweenLite.to(this,0.5,{
						"x":_loc2,
						"onComplete":this.onTweenComplete
					});
		...
		private function updateLayout() : void
		{
			...
			if(this._status.hasStatus(DockDef.STATUS_SHARE_SHOW))
			{
				...
				this._dockUI.shareBtn.y = _loc5;
				_loc5 = _loc5 + (_loc1 + _loc2 * 2 + _loc6);
				_loc3++;
			}
			else
			{
			...
			if(this._status.hasStatus(DockDef.STATUS_LIGHT_SHOW))
			{
				...
				this._dockUI.openLightBtn.y = _loc5;
				this._dockUI.closeLightBtn.y = _loc5;
				_loc5 = _loc5 + (_loc1 + _loc2 * 2 + _loc6);
				_loc3++;
			}
			else
			...
			if(this._status.hasStatus(DockDef.STATUS_OFFLINE_WATCH_SHOW))
			{
				...
				this._dockUI.offlineWatchBtn.y = _loc5;
				this._dockUI.copyrightLimitBtn.y = _loc5;
				_loc5 = _loc5 + (_loc1 + _loc2 * 2 + _loc6);
				_loc3++;
			}
			else
			...
			var _loc7:DisplayObject = null;
			_loc5 = _loc1 + _loc2;
			var _loc8:* = 0;
			while(_loc8 < _loc4 - 1)
			{
				_loc7 = this._dockUI.getChildByName("line" + _loc8);
				if(_loc8 >= _loc3 - 1)
				{
					_loc7.visible = false;
				}
			...
			{
				this._bg.height = _loc3 * (_loc1 + _loc2 * 2 + _loc6) - _loc2 * 2 - _loc6;
			}
