package com.qiyi.player.wonder.plugins.scenetile.model.barrage.socket
{
	...
	public class BarrageSocket extends EventDispatcher
	{
		...
		private function onSocketDataHandler(param1:ProgressEvent) : void
		{
			...
			{
				this._socketData.position = 0;
				_loc2 = this._socketData.readUnsignedShort();
				_loc3 = this._socketData.readUnsignedShort();
				_loc4 = this._socketData.readUnsignedInt();
				_loc5 = this._socketData.length - this.HEAD_BYTE_SIZE - _loc4;
				if(_loc5 >= 0)
				{
					this.parse(_loc3);
					if(_loc5 > 0)
					{
						this._cacheData.writeBytes(this._socketData,this.HEAD_BYTE_SIZE + _loc4);
						this._socketData.position = 0;
						this._socketData.length = 0;
		...
		private function parse(param1:int) : void
		{
			...
			if(param1 > 0)
			{
				...
				while(_loc3 < param1)
				{
					_loc4 = this._socketData.readUnsignedByte();
					_loc5 = this._socketData.readUnsignedInt();
					_loc6 = this._socketData.readMultiByte(_loc5,"utf-8");
					_loc7 = new Object();
					_loc7.TVLType = _loc4;
					_loc7.TVLContent = _loc6;
					_loc2.push(_loc7);
					_loc3++;
				}
				dispatchEvent(new CommonEvent(Evt_BarrageSocketReceiveData,_loc2));
			}
		}
		...
		public function send(param1:int, param2:String) : void
		{
			var _loc3:* = 0;
			var _loc4:* = 0;
			var _loc5:ByteArray = null;
			if(this._socket.connected)
			{
				_loc3 = 1;
				_loc4 = 1;
				_loc5 = new ByteArray();
				_loc5.endian = Endian.BIG_ENDIAN;
				_loc5.writeMultiByte(param2,"utf-8");
				this._socket.writeShort(_loc3);
				this._socket.writeShort(_loc4);
				this._socket.writeInt(this.TVL_TYPE_BYTE_SIZE + this.TVL_CONTENTSIZE_BYTE_SIZE + _loc5.length);
				this._socket.writeByte(param1);
				this._socket.writeInt(_loc5.length);
				this._socket.writeBytes(_loc5);
				this._socket.flush();
			}
		}
		...
		private function createSendContent(param1:int, param2:Boolean = true) : String
		{
			...
			{
				_loc3 = UserManager.getInstance().user.P00001;
			}
			var _loc4:Object = new Object();
			_loc4.msgType = param1;
			var _loc5:Object = new Object();
			_loc5.tvid = Number(this._tvid);
			_loc5.addTime = new Date().getTime();
			_loc5.authcookie = _loc3;
			_loc5.udid = UUIDManager.instance.uuid;
			_loc5.appid = APPID;
			_loc5.history = param2?1:0;
			_loc4.data = [_loc5];
