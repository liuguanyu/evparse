package com.qiyi.player.wonder.plugins.controllbar.view.seekbar
{
	...
	public class SeekBarView extends Sprite
	{
		...
		private function onBarBgHeightChange() : void
		{
			...
			var _loc1:* = 0;
			while(_loc1 < this._viewPoints.length)
			{
				this.onResizePoint(this._viewPoints[_loc1]);
				_loc1++;
			}
			var _loc2:* = 0;
			while(_loc2 < this._merchandiseViewPoints.length)
			{
				this.onResizePoint(this._merchandiseViewPoints[_loc2]);
				_loc2++;
			}
		}
		...
		private function drawFilterBar() : void
		{
			...
			if(!(this._filterData == null) && this._filterData.length > 0)
			{
				...
				_loc1 = 0;
				while(_loc1 < this._filterData.length)
				{
					this.resizeSkipPart(this._filterData[_loc1]);
					_loc1++;
				}
		...
		{
			var _loc3:* = 0;
			var _loc4:FocusPoint = null;
			var _loc2:* = 0;
			if(this._viewPoints)
			{
				_loc3 = this._viewPoints.length;
				_loc2 = 0;
				while(_loc2 < _loc3)
				{
					this._viewPoints[_loc2].removeEventListener(MouseEvent.ROLL_OVER,this.onViewPointRollOver);
					this._viewPoints[_loc2].removeEventListener(MouseEvent.ROLL_OUT,this.onViewPointRollOut);
					this._viewPoints[_loc2].addEventListener(MouseEvent.CLICK,this.onViewPointClick);
					removeChild(this._viewPoints[_loc2]);
					_loc2++;
				}
				this._viewPoints = new Vector.<FocusPoint>();
			}
			if(!(param1 == null) && param1.length > 0)
			{
				...
				{
					_loc4 = new FocusPoint();
					_loc4.buttonMode = true;
					_loc4.useHandCursor = true;
					_loc4.time = param1[_loc2].timestamp;
					_loc4.content = param1[_loc2].content;
					_loc4.addEventListener(MouseEvent.ROLL_OVER,this.onViewPointRollOver);
					_loc4.addEventListener(MouseEvent.ROLL_OUT,this.onViewPointRollOut);
					_loc4.addEventListener(MouseEvent.CLICK,this.onViewPointClick);
					this._viewPoints.push(_loc4);
					addChild(_loc4);
					_loc2++;
				}
			}
		...
		public function setMerchandiseViewPoints(param1:Array) : void
		{
			...
			{
				_loc3 = this._merchandiseViewPoints.length;
				_loc2 = 0;
				while(_loc2 < _loc3)
				{
					this._merchandiseViewPoints[_loc2].removeEventListener(MouseEvent.ROLL_OVER,this.onViewPointRollOver);
					this._merchandiseViewPoints[_loc2].removeEventListener(MouseEvent.ROLL_OUT,this.onViewPointRollOut);
					this._merchandiseViewPoints[_loc2].addEventListener(MouseEvent.CLICK,this.onViewPointClick);
					removeChild(this._merchandiseViewPoints[_loc2]);
					_loc2++;
				}
				this._merchandiseViewPoints = new Vector.<FocusPoint>();
			}
			if(!(param1 == null) && param1.length > 0)
			{
				...
				while(_loc2 < param1.length)
				{
					...
					_loc4.buttonMode = true;
					_loc4.useHandCursor = true;
					_loc4.time = Number(param1[_loc2].startTime) * 1000;
					_loc4.content = param1[_loc2].promotion;
					_loc4.imgUrl = param1[_loc2].imgUrl;
					_loc4.goodsUrl = param1[_loc2].detailUrl;
					_loc4.addEventListener(MouseEvent.ROLL_OVER,this.onViewPointRollOver);
					_loc4.addEventListener(MouseEvent.ROLL_OUT,this.onViewPointRollOut);
					_loc4.addEventListener(MouseEvent.CLICK,this.onViewPointClick);
					this._merchandiseViewPoints.push(_loc4);
					addChild(_loc4);
					_loc2++;
				}
			}
		...
		private function exResize(param1:int, param2:int) : void
		{
			...
			if(this._status.hasStatus(ControllBarDef.STATUS_SEEK_BAR_THICK))
			{
				...
				_loc3 = 0;
				while(_loc3 < this._viewPoints.length)
				{
					this.onResizePoint(this._viewPoints[_loc3]);
					_loc3++;
				}
				_loc4 = 0;
				while(_loc4 < this._merchandiseViewPoints.length)
				{
					this.onResizePoint(this._merchandiseViewPoints[_loc4]);
					_loc4++;
				}
				if(!(this._filterData == null) && this._filterData.length > 0)
				{
					...
					this._filterShape.graphics.beginFill(FILTER_COLOUR,0.6);
					_loc5 = 0;
					while(_loc5 < this._filterData.length)
					{
						this.resizeSkipPart(this._filterData[_loc5]);
						_loc5++;
					}
					this._filterShape.graphics.endFill();
				}
		...
		private function showImagePreview(param1:Number, param2:Number, param3:String = "", param4:String = "") : void
		{
			...
			if(!this._isAlreadyLoadImageData)
			{
				...
				_loc5 = 0;
				while(_loc5 < this._imagePrePicUrlArr.length)
				{
					PreviewImageLoader.instance.imgLoader(this._imagePrePicUrlArr[_loc5]);
					_loc5++;
				}
			}
		...
		{
			var _loc2:Number = this._seekBarBg.height * 0.4;
			param1.resize(_loc2 > 2?2:_loc2);
		...
		private function resizeSkipPart(param1:ISkipPointInfo) : void
		{
			...
			var _loc2:int = this._seekBarBg.x + param1.startTime / this._totalTime * this._seekBarBg.width;
			var _loc3:int = this._seekBarBg.y;
			var _loc4:int = this._seekBarBg.height;
			var _loc5:int = (param1.endTime - param1.startTime) / this._totalTime * this._seekBarBg.width;
			this._filterShape.graphics.drawRect(_loc2,_loc3,_loc5,_loc4);
		}
		
		private function onFocusPointHeadTailRollOver(param1:MouseEvent) : void
		{
			var _loc2:FocusPoint = null;
			var _loc3:* = 0;
			if(this._videoHeadTailTipPanel.parent == null)
			{
				_loc2 = param1.currentTarget as FocusPoint;
				this._videoHeadTailTipPanel.y = localToGlobal(new Point(0,_loc2.y - this._videoHeadTailTipPanel.height)).y;
				_loc3 = localToGlobal(new Point(_loc2.x + _loc2.width / 2 - this._videoHeadTailTipPanel.width / 2,0)).x;
				if(_loc3 <= 0)
				{
					_loc3 = _loc3 + 15;
				}
				if(_loc3 >= GlobalStage.stage.stageWidth - this._videoHeadTailTipPanel.width - 1)
				{
					_loc3 = GlobalStage.stage.stageWidth - this._videoHeadTailTipPanel.width - 5;
				}
				this._videoHeadTailTipPanel.x = _loc3;
		...
		{
			var _loc2:FocusPoint = param1.currentTarget as FocusPoint;
			if(!(mouseX >= _loc2.x + _loc2.width / 2 - 4 && mouseX <= _loc2.x + _loc2.width / 2 + 4) || mouseY >= _loc2.y)
			{
		...
		{
			var _loc2:int = globalToLocal(new Point(param1.stageX,param1.stageY)).x - this._seekBarBg.x;
			var _loc3:int = _loc2 / this._seekBarBg.width * this._totalTime;
			dispatchEvent(new ControllBarEvent(ControllBarEvent.Evt_Seek,_loc3));
			this._seekClickCount++;
		}
		...
		{
			var _loc2:FocusPoint = param1.currentTarget as FocusPoint;
			dispatchEvent(new ControllBarEvent(ControllBarEvent.Evt_Seek,_loc2.time));
			this._seekClickCount++;
		}
		...
		private function onStageMouseMove(param1:MouseEvent) : void
		{
			...
			var _loc2:Number = mouseX;
			if(_loc2 < this._backWardBtn.width)
			{
				_loc2 = 0;
			}
			if(_loc2 > this._seekBarBg.x + this._seekBarBg.width)
			{
				_loc2 = this._seekBarBg.x + this._seekBarBg.width;
			}
			this.drawPlayBar(this._seekBarBg.x,this._seekBarBg.y,_loc2 - this._seekBarBg.x,this._seekBarBg.height);
			this._slider.x = _loc2 - this._slider.width / 2;
			this.checkSliderBorder();
			this._mouseMoved = true;
			var _loc3:int = 1 * (_loc2 - this._seekBarBg.x) / this._seekBarBg.width * this._totalTime;
			if(_loc3 > this._totalTime)
			{
				_loc3 = this._totalTime;
			}
			if(_loc3 < 0)
			{
				_loc3 = 0;
			}
			this.showImagePreview(_loc3,_loc2);
		...
		private function onStageMouseUp(param1:MouseEvent) : void
		{
			...
			var _loc2:int = 1 * this._playBar.width / this._seekBarBg.width * this._totalTime;
			dispatchEvent(new ControllBarEvent(ControllBarEvent.Evt_Seek,_loc2));
		...
		private function onSeekBarBgMouseMove(param1:MouseEvent) : void
		{
			...
			var _loc2:int = 1 * (mouseX - this._seekBarBg.x) / this._seekBarBg.width * this._totalTime;
			this.showImagePreview(_loc2,mouseX);
			this.checkImagePreBound();
		}
		...
		private function onViewPointRollOver(param1:MouseEvent) : void
		{
			...
			this._mouseMoveShape.graphics.clear();
			var _loc2:FocusPoint = param1.currentTarget as FocusPoint;
			this.showImagePreview(_loc2.time,_loc2.x + _loc2.width / 2 - 2,_loc2.content,_loc2.imgUrl);
		...
		private function checkImagePreBound(param1:Boolean = false) : void
		{
			...
			{
				_loc2 = this._imagePreview.x - ControllBarDef.IMAGE_PRE_BIG_WH_SIZE.x * 0.5;
				if(_loc2 <= 0)
				{
					this._imagePreview.x = ControllBarDef.IMAGE_PRE_BIG_WH_SIZE.x * 0.5;
				}
				_loc2 = this._imagePreview.x + ControllBarDef.IMAGE_PRE_BIG_WH_SIZE.x * 0.5;
				if(_loc2 >= GlobalStage.stage.stageWidth)
				{
			...
			{
				_loc2 = this._imagePreview.x - ControllBarDef.IMAGE_PRE_TIME_SIZE.x * 0.5;
				if(_loc2 <= 0)
				{
