package com.qiyi.player.wonder.plugins.continueplay.view.parts
{
	...
	public class ContinueItemView extends Sprite
	{
		...
		private function checkVideoPublishTime(param1:String) : String
		{
			...
			{
				while(_loc4.indexOf("-") != -1)
				{
					_loc4 = _loc4.replace("-","/");
				}
				_loc5 = _loc2.getFullYear() + "/" + (_loc2.getMonth() + 1) + "/" + _loc2.getDate();
				_loc6 = Date.parse(_loc2.getFullYear() + "/" + (_loc2.getMonth() + 1) + "/" + _loc2.getDate());
				_loc7 = Date.parse(_loc4);
				_loc8 = 86400000;
				if(_loc7 - _loc6 >= 0)
				{
					if(int(param1.split(" ")[1].split(":")[0]) > 0 || int(param1.split(" ")[1].split(":")[1]) > 0 || int(param1.split(" ")[1].split(":")[2]) > 0)
					{
						if(_loc2.time - _loc7 >= 0 && _loc2.time - _loc7 <= _loc8 / 24)
						{
							_loc3 = _loc3 + (Math.floor((_loc2.time - _loc7) / 60000) + "分钟前");
						}
						else if(_loc2.time - _loc7 > _loc8 / 24)
						{
							_loc3 = _loc3 + ("今天" + param1.split(" ")[1].split(":")[0] + ":" + param1.split(" ")[1].split(":")[1]);
						}
						else
						{
							_loc3 = _loc3 + param1.split(" ")[0];
						}
				...
				else if(Math.abs(_loc7 - _loc6) <= _loc8)
				{
					_loc3 = _loc3 + "昨天";
				}
				else if(Math.abs(_loc7 - _loc6) > _loc8 && Math.abs(_loc7 - _loc6) <= _loc8 * 2)
				{
					_loc3 = _loc3 + "前天";
				}
				else
