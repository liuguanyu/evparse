package com.qiyi.player.wonder.plugins.scenetile.model
{
	...
	public class SceneTileProxy extends Proxy implements IStatus
	{
		...
		private function analyzeXmlBarrageData(param1:XML) : void
		{
			...
			var _loc4:XML = null;
			var _loc5:XML = null;
			for each(_loc4 in param1.data.entry)
			{
				_loc2 = this._vecOP.pop() as Vector.<BarrageInfo>;
				if(_loc2 == null)
				{
					_loc2 = new Vector.<BarrageInfo>();
				}
				for each(_loc5 in _loc4.list.bulletInfo)
				{
					_loc3 = this._barrageItemOP.pop() as BarrageInfo;
					if(_loc3 == null)
					{
						_loc3 = new BarrageInfo();
					}
					_loc3.update(_loc5.contentId,_loc5.showTime,_loc5.content,_loc5.likes,_loc5.font == undefined?SceneTileDef.BARRAGE_DEFAULT_FONT_SIZE:_loc5.font,_loc5.color == undefined?SceneTileDef.BARRAGE_DEFAULT_FONT_COLOR:_loc5.color,_loc5.position == undefined?SceneTileDef.BARRAGE_POSITION_NONE:_loc5.position,_loc5.contentType == undefined?SceneTileDef.BARRAGE_CONTENT_TYPE_NONE:_loc5.contentType,_loc5.background == undefined?SceneTileDef.BARRAGE_BG_TYPE_NONE:_loc5.background,_loc5.userInfo == undefined?null:_loc5.userInfo);
					_loc2.push(_loc3);
				}
				this._barrageData[uint(_loc4.int)] = _loc2;
			}
		}
		
		private function clearBarrageData() : void
		{
			var _loc1:Vector.<BarrageInfo> = null;
			var _loc2:BarrageInfo = null;
			var _loc3:* = undefined;
			for(_loc3 in this._barrageData)
			{
				_loc1 = this._barrageData[_loc3] as Vector.<BarrageInfo>;
				while(_loc1.length > 0)
				{
					_loc2 = _loc1.pop();
					this._barrageItemOP.push(_loc2);
				}
				this._vecOP.push(_loc1);
				delete this._barrageData[_loc3];
				true;
			}
		}
		...
		public function getBarrageData(param1:int) : Vector.<BarrageInfo>
		{
			...
			var _loc2:Vector.<BarrageInfo> = this._barrageData[param1];
			if((_loc2) && (this._barrageSocket.connected))
			{
				_loc3 = new Vector.<BarrageInfo>();
				_loc4 = 0;
				while(_loc4 < _loc2.length)
				{
					if(_loc2[_loc4].contentType != SceneTileDef.BARRAGE_CONTENT_TYPE_STAR)
					{
		...
		private function onLoaderBIIComplete(param1:CommonEvent = null) : void
		{
			...
			_loc2.callJsSetBarrageInteractInfo(this._starInteractInfo.starInteractObj,this._barrageSocket.connected);
			var _loc3:PlayerProxy = facade.retrieveProxy(PlayerProxy.NAME) as PlayerProxy;
			if(_loc3)
			{
				_loc3.curActor.isStarBarrage = _loc3.preActor.isStarBarrage = this._barrageSocket.connected;
			}
		}
