package com.qiyi.player.wonder.plugins.feedback.view.parts.copyrightexpired {
	...
	public class CopyrightExpiredListPart extends Sprite {
		...
		public function set recommendData(param1:Vector.<RecommendVO>) : void {
			var _loc2_:CopyrightExpiredRecommendItem = null;
			var _loc3_:uint = 0;
			this._recommendData = param1;
			if(this._recommendData) {
				_loc3_ = 0;
				while(_loc3_ < this._recommendData.length) {
					_loc2_ = new CopyrightExpiredRecommendItem(this._recommendData[_loc3_]);
					_loc2_.x = (_loc2_.width + GAP_OF_ITEM) * _loc3_ + GAP_OF_ITEM * 0.5;
					this._itemVector.push(_loc2_);
					this._listContainer.addChild(_loc2_);
					_loc2_.addEventListener(MouseEvent.CLICK,this.onItemMouseClick);
					_loc3_++;
				}
			}
		...
		private function onItemMouseClick(param1:MouseEvent) : void {
			var _loc2_:CopyrightExpiredRecommendItem = param1.target as CopyrightExpiredRecommendItem;
			if(_loc2_) {
				dispatchEvent(new FeedbackEvent(FeedbackEvent.Evt_CopyrightRecItemClick,_loc2_.data));
			}
		}
		...
		private function onLeftArrowClick(param1:MouseEvent = null) : void {
			...
			if(this._currPage > 0 && !this._isMove) {
				...
				this._isMove = true;
				_loc2_ = this._listContainer.x + (CopyrightExpiredRecommendItem.ITEM_WIDTH + GAP_OF_ITEM) * this._maxShowItem;
		...
		public function resize(param1:Number, param2:Number) : void {
			...
			_loc3_ = GAP_OF_BTN * 2 + this._maxShowItem * (CopyrightExpiredRecommendItem.ITEM_WIDTH + GAP_OF_ITEM);
			_loc4_ = (param1 - _loc3_) / 2;
			this._leftArrow.x = _loc4_ + this._leftArrow.width + (GAP_OF_BTN - this._leftArrow.width) / 2;
			this._rightArrow.x = _loc4_ + _loc3_ - this._rightArrow.width - (GAP_OF_BTN - this._leftArrow.width) / 2;
			this._leftArrow.y = this._rightArrow.y = (ITEM_HEIGHT - this._rightArrow.height) * 0.5 - 5;
			this._listContainer.x = _loc4_ + GAP_OF_BTN;
			this._listContainerMask.x = _loc4_ + GAP_OF_BTN;
			this._listContainerMask.width = _loc3_ - GAP_OF_BTN * 2;
		...
		public function destory() : void {
			var _loc1_:CopyrightExpiredRecommendItem = null;
