	...
	public class BaseEngine extends EventDispatcher implements IEngine
	{
		...
		public function seek(param1:uint, param2:int = 0) : void
		{
			...
			else
			{
				...
				var _loc3:Segment = this._movie.curSegment;
				var _loc4:Keyframe = _loc3.currentKeyframe;
				if(this._movie.streamType == StreamEnum.HTTP)
				{
					if(_loc4)
					{
						if(_loc3.keyframes.length >= 2 && _loc4.index == _loc3.keyframes.length - 1)
						{
							_loc6 = _loc3.keyframes[_loc3.keyframes.length - 2];
							this._seekTime = _loc6.time;
							this._movie.seek(this._seekTime);
							_loc3 = this._movie.curSegment;
							_loc4 = _loc3.currentKeyframe;
							this._seekTime = _loc4.time;
						}
						else
						{
							this._seekTime = _loc4.time;
					...
					{
						this._seekTime = _loc3.startTime;
					}
				}
				else
				{
				...
				var _loc5:int = this._holder.runtimeData.endTime > 0?this._holder.runtimeData.endTime:this._movie.duration;
				if(_loc5 - this._seekTime < this._holder.runtimeData.prepareToPlayEnd)
				{
				...
				if(Settings.instance.skipTrailer)
				{
					if(this._movie.streamType == StreamEnum.HTTP)
					{
						if((this._movie.trailerTime > 0) && (_loc4) && _loc4.time < this._movie.trailerTime)
						{
				...
				if(this._decoder)
				{
					if(_loc5 - this._seekTime < Config.STREAM_NORMAL_BUFFER_TIME && _loc5 - this._seekTime > 0)
					{
		...
		protected function setStatus(param1:int) : void
		{
			...
			if(!this._holder.hasStatus(param1))
			{
				...
				{
					this._playingDurationStartTime = _loc3;
					this._seekTime = -1;
				}
				else
				{
					...
					{
						this._playingDuration = this._playingDuration + (_loc3 - this._playingDurationStartTime);
					}
					this._playingDurationStartTime = 0;
				}
		...
		protected function onTimer(param1:TimerEvent) : void
		{
			...
			{
				if((this._holder.runtimeData.recordHistory && _loc2 > 1000) && (this._holder.history) && (this._holder.hasStatus(StatusEnum.ALREADY_PLAY)))
				{
					this._holder.history.update(_loc2);
				}
			...
			if(_loc2 > this._movie.duration + 100000)
			{
				this._log.warn("Decoder currentTime error! currentTime:" + _loc2 + ",oldCurrentTime:" + this._oldCurrentTime + ",duration:" + this._movie.duration);
				this.seek(this._oldCurrentTime);
				return;
			}
			this._oldCurrentTime = _loc2;
			var _loc3:int = this._movie.skipPointInfoCount;
			var _loc4:SkipPointInfo = null;
			var _loc5:* = 0;
			while(_loc5 < _loc3)
			{
				_loc4 = this._movie.getSkipPointInfoAt(_loc5) as SkipPointInfo;
				if(_loc4 == null)
				{
					break;
				}
				if(this._holder.runtimeData.prepareToSkipPoint > 0 && _loc4.startTime - _loc2 <= this._holder.runtimeData.prepareToSkipPoint && _loc4.startTime - _loc2 > 0)
				{
					_loc4.inCurPrepareSkipPoint = true;
					_loc4.outPrepareSkipPointFlag = false;
					if(!_loc4.enterPrepareSkipPointFlag)
					{
						_loc4.enterPrepareSkipPointFlag = true;
						dispatchEvent(new EngineEvent(EngineEvent.Evt_EnterPrepareSkipPoint,_loc4));
					}
				}
				else
				{
					if(_loc4.inCurPrepareSkipPoint)
					{
						if(!_loc4.outPrepareSkipPointFlag)
						{
							_loc4.outPrepareSkipPointFlag = true;
							dispatchEvent(new EngineEvent(EngineEvent.Evt_OutPrepareSkipPoint,_loc4));
						}
					}
					_loc4.inCurPrepareSkipPoint = false;
					_loc4.enterPrepareSkipPointFlag = false;
				}
				if(this._holder.runtimeData.prepareLeaveSkipPoint > 0 && _loc4.endTime - _loc2 < this._holder.runtimeData.prepareLeaveSkipPoint && _loc4.endTime - _loc2 >= 0)
				{
					_loc4.inCurPrepareLeaveSkipPoint = true;
					_loc4.outPrepareLeaveSkipPointFlag = false;
					if(!_loc4.enterPrepareLeaveSkipPointFlag)
					{
						_loc4.enterPrepareLeaveSkipPointFlag = true;
						dispatchEvent(new EngineEvent(EngineEvent.Evt_EnterPrepareLeaveSkipPoint,_loc4));
					}
				}
				else
				{
					if(_loc4.inCurPrepareLeaveSkipPoint)
					{
						if(!_loc4.outPrepareLeaveSkipPointFlag)
						{
							_loc4.outPrepareLeaveSkipPointFlag = true;
							dispatchEvent(new EngineEvent(EngineEvent.Evt_OutPrepareLeaveSkipPoint,_loc4));
						}
					}
					_loc4.inCurPrepareLeaveSkipPoint = false;
					_loc4.enterPrepareLeaveSkipPointFlag = false;
				}
				if(_loc2 >= _loc4.startTime && _loc2 < _loc4.endTime)
				{
					_loc4.inCurSkipPoint = true;
					_loc4.outSkipPointFlag = false;
					if(!_loc4.enterSkipPointFlag)
					{
						_loc4.enterSkipPointFlag = true;
						dispatchEvent(new EngineEvent(EngineEvent.Evt_EnterSkipPoint,_loc4));
					}
				}
				else
				{
					if(_loc4.inCurSkipPoint)
					{
						if(!_loc4.outSkipPointFlag)
						{
							_loc4.outSkipPointFlag = true;
							dispatchEvent(new EngineEvent(EngineEvent.Evt_OutSkipPoint,_loc4));
						}
					}
					_loc4.inCurSkipPoint = false;
					_loc4.enterSkipPointFlag = false;
				}
				_loc5++;
			}
			if((this._holder.hasStatus(StatusEnum.STOPPING)) || (this._holder.hasStatus(StatusEnum.STOPED)) || (this._holder.hasStatus(StatusEnum.FAILED)))
			...
			if(this._holder.runtimeData.originalEndTime > 0)
			{
				_loc6 = this._holder.runtimeData.originalEndTime;
			}
			else if(this._movie.trailerTime > 0 && (this._holder.runtimeData.skipTrailer))
			{
				_loc6 = this._movie.trailerTime;
			}
			else
			{
				_loc6 = this._movie.duration;
			}
			
			if(this._holder.runtimeData.prepareToPlayEnd > 0 && _loc6 - _loc2 <= this._holder.runtimeData.prepareToPlayEnd)
			{
				if(this._needDispatchPrepareStop)
				{
					this._needDispatchPrepareStop = false;
					this._log.info("prepare to end,curTime:" + _loc2 + ",duration:" + this._movie.duration + ",endTime:" + _loc6);
			...
			if(this._holder.runtimeData.originalEndTime > 0)
			{
				if(_loc2 >= this._holder.runtimeData.originalEndTime)
				{
					this._log.info("arrive at endTime,curTime:" + _loc2 + ",duration:" + this._movie.duration + ",appdata.endTime:" + this._holder.runtimeData.originalEndTime);
			...
			{
				if(_loc2 >= this._movie.trailerTime)
				{
					this._log.info("skip trailer,curTime:" + _loc2 + ",duration:" + this._movie.duration + ",trailerTime:" + this._movie.trailerTime);
		...
		{
			var _loc1:Boolean = (this._movie) && (this._movie.curDefinition) && (this._movie.curDefinition.ready);
			var _loc2:Boolean = (this._holder.history) && (this._holder.history.getReady()) || !(this._holder.runtimeData.playerUseType == PlayerUseTypeEnum.MAIN);
			return (_loc1) && (_loc2);
		}
