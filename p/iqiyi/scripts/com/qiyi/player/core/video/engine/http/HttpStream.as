package com.qiyi.player.core.video.engine.http
{
	...
	public class HttpStream extends EventDispatcher implements IDestroy
	{
		...
		public function get bufferTime() : Number
		{
			...
			var _loc1:Number = this._startKeyframe.position + this._ns.bytesLoaded - this._segment.keyframes[0].position;
			var _loc2:Keyframe = this._segment.getKeyframeByPosition(_loc1);
			return _loc2.time + (_loc1 - _loc2.position) / _loc2.lenPos * _loc2.lenTime;
		}
		
		public function get bufferRate() : Number
		{
			var _loc1:Number = this._ns?this._ns.bufferLength / this._ns.bufferTime:0;
			return _loc1 >= 1?1:_loc1;
		}
		...
		public function seek(param1:int = -1) : void
		{
			...
			if(this._ns)
			{
				_loc2 = this._segment.currentKeyframe;
				if(_loc2 == null)
				{
					this._seeking = false;
					this.reload();
				...
				{
					_loc3 = this._startKeyframe?this._startKeyframe.position:0;
					if(_loc2.position >= _loc3 && _loc2.position < _loc3 + this._ns.bytesLoaded)
					{
						this._ns.seek(_loc2.segmentTime);
					}
					else
					{
						this.reload();
		...
		{
			var _loc2:String = param1.data as String;
			this._ns.play(_loc2);
		}
