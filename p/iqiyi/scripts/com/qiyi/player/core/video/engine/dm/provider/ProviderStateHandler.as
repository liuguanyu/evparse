package com.qiyi.player.core.video.engine.dm.provider
{
	...
	public class ProviderStateHandler extends Object
	{
		...
		public function onFinalError(param1:Provider) : void
		{
			...
			var _loc2:int = param1.fileState.stateCode;
			var _loc3:String = param1.fileState.data;
			this._log.error("Provider.onFinalError, code = " + _loc2);
			if(_loc2 == FileState.FirstDispatch + FileState.State_Timeout)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.FirstDispatch + FileState.State_ConnectError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.FirstDispatch + FileState.State_DataError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.FirstDispatch + FileState.State_SecurityError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.SecondDispatch + FileState.State_Timeout)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.SecondDispatch + FileState.State_ConnectError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.SecondDispatch + FileState.State_DataError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.SecondDispatch + FileState.State_SecurityError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthChecker + FileState.State_Timeout)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthChecker + FileState.State_ConnectError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthChecker + FileState.State_DataError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthChecker + FileState.State_SecurityError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthDispatch + FileState.State_Timeout)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthDispatch + FileState.State_ConnectError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthDispatch + FileState.State_DataError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.AuthDispatch + FileState.State_SecurityError)
			{
				this._holder.pingBack.sendError(this._holder.runtimeData.errorCode);
				return;
			}
			if(_loc2 == FileState.CDNRequest + FileState.State_Timeout)
			{
				return;
			}
			if(_loc2 == FileState.CDNRequest + FileState.State_ConnectError)
			{
				return;
			}
			if(_loc2 == FileState.CDNRequest + FileState.State_DataError)
			{
				return;
			}
			if(_loc2 == FileState.CDNRequest + FileState.State_SecurityError)
			{
				return;
			}
		}
