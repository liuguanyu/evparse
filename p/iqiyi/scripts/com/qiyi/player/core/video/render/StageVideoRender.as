	...
	public class StageVideoRender extends Render
	{
		...
		override public function tryUseGPU() : void
		{
			...
			if((_holder.runtimeData.supportGPU) && (_decoder))
			{
				...
				{
					_loc1 = StageVideoManager.instance.stageVideoIsAvailable;
				}
				if(_loc1 != this._stageVideoInUse)
				{
					this._stageVideoInUse = _loc1;
					this.setUseGPU();
				}
		...
		{
			var _loc1:Point = null;
			if((this._stageVideo) && (_realArea.width > 0) && _realArea.height > 0)
			{
				_loc1 = _parent.localToGlobal(new Point(_realArea.x,_realArea.y));
				this._stageVideo.viewPort = new Rectangle(_loc1.x,_loc1.y,_realArea.width,_realArea.height);
			}
		}
