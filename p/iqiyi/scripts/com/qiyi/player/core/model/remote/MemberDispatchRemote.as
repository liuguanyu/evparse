package com.qiyi.player.core.model.remote
{
	...
	public class MemberDispatchRemote extends BaseRemoteObject
	{
		...
		{
			var _loc1:String = this._segment.url.split(".f4v").join(".hml");
			_loc1 = _loc1 + ("?t=" + this._holder.runtimeData.key + "&cid=" + this._holder.runtimeData.communicationlId + "&vid=" + this._holder.runtimeData.vid);
			_loc1 = _loc1 + ("&QY00001=" + this._holder.runtimeData.QY00001);
			if(this._startPos == -1)
			{
				if((this._segment.currentKeyframe) && !(this._segment.currentKeyframe.index == 0))
				{
					_loc1 = _loc1 + ("&start=" + this._segment.currentKeyframe.position);
				}
			}
