package com.qiyi.player.core.model.remote {
	...
	public class RequestEnjoyableSkipPointsRemote extends BaseRemoteObject {
		...
		private function parseData(param1:Array) : Vector.<SkipPointInfo> {
			...
			var _loc2_:SkipPointInfo = null;
			var _loc3_:Vector.<SkipPointInfo> = new Vector.<SkipPointInfo>();
			var _loc4_:* = 0;
			while(_loc4_ < param1.length) {
				_loc5_ = param1[_loc4_];
				if(_loc5_) {
					_loc2_ = new SkipPointInfo();
					if(int(_loc5_.type) == 1) {
						_loc2_.skipPointType = SkipPointEnum.ENJOYABLE;
					}
					_loc2_.startTime = int(_loc5_.start) * 1000;
					_loc2_.endTime = int(_loc5_.end) * 1000;
					_loc3_.push(_loc2_);
				}
				_loc4_++;
			}
