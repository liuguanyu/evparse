	...
	public class Segment extends Object implements ISegment, IDestroy
	{
		...
		public function Segment(param1:ICorePlayer, param2:String, param3:int, param4:Number, param5:Number, param6:String, param7:Number, param8:Number)
		{
			...
			if(!(this._url == "") && !(this._url == null))
			{
				...
				{
					_loc10 = "http://" + this._holder.runtimeData.cacheServerIP + "/";
					this._url = this._url.replace(new RegExp("http:\\/\\/(\\w|\\.)*\\/"),_loc10);
				}
				_loc9 = this._url.split("/");
				if((_loc9) && _loc9.length > 0)
				{
					_loc11 = _loc9[_loc9.length - 1];
					_loc9 = _loc11.split(".");
					if((_loc9) && _loc9.length > 0)
					{
						this._rid = _loc9[0];
		...
		{
			var _loc3:Keyframe = null;
			var _loc4:Keyframe = null;
			var _loc5:* = 0;
			var _loc6:* = 0;
			var _loc2:Vector.<Keyframe> = new Vector.<Keyframe>();
			if(this._keyframes)
			{
				_loc3 = null;
				_loc4 = null;
				_loc5 = this._keyframes.length;
				_loc6 = 1;
				while(_loc6 < _loc5)
				{
					_loc4 = this._keyframes[_loc6];
					if(_loc4.time >= param1)
					{
						_loc2.push(this._keyframes[_loc6 - 1]);
						_loc2.push(this._keyframes[_loc6]);
						break;
					}
					if(_loc6 == _loc5 - 1)
					{
		...
		public function getKeyframeByTime(param1:Number) : Keyframe
		{
			...
			{
				_loc2 = 1;
				_loc3 = this._keyframes.length;
				while(_loc2 < _loc3)
				{
					if(this._keyframes[_loc2].time >= param1)
					{
						return Math.abs(this._keyframes[_loc2].time - param1) > Math.abs(this._keyframes[_loc2 - 1].time - param1)?this._keyframes[_loc2 - 1]:this._keyframes[_loc2];
					}
					_loc2++;
				}
			}
		...
		public function getKeyframeByPosition(param1:Number) : Keyframe
		{
			...
			if(param1 >= 0 && param1 < this.totalBytes)
			{
				...
				while(_loc2 < _loc3)
				{
					if(this._keyframes[_loc2].position == param1)
					{
						return this._keyframes[_loc2];
					}
					if(this._keyframes[_loc2].position > param1)
					{
						return this._keyframes[_loc2 - 1];
					}
					_loc2++;
				}
			}
		...
		{
			var _loc2:Keyframe = this.getKeyframeByTime(param1);
			if(_loc2)
			{
				this._currentKeyframeIndex = _loc2.index;
				if((this._holder) && (this._holder.runtimeData.originalEndTime > 0) && _loc2.time > this._holder.runtimeData.originalEndTime)
				{
					this._currentKeyframeIndex--;
				}
				_loc2 = this._keyframes[this._currentKeyframeIndex];
				if(this._totalBytes - _loc2.position < 10000)
				{
					this._currentKeyframeIndex--;
				}
			}
