		...
		private function onTimer(param1:TimerEvent) : void {
			...
			if(this._corePlayer) {
				if(this._corePlayer.hasStatus(StatusEnum.PLAYING)) {
					_loc2_ = this._corePlayer.currentTime;
					if(_loc2_ != this._blackScreenCurTime) {
						this._blackScreenCurTime = _loc2_;
					...
					if(this._blackScreenTime >= this.BLACK_SCREEN_TIME) {
						this._log.warn("CorePlayer arrive black limit time,execute seek,seek time:" + _loc2_);
						this._corePlayer.seek(_loc2_);
						this._blackScreenTime = 0;
