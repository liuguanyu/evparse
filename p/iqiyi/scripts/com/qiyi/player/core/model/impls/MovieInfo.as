package com.qiyi.player.core.model.impls {
	...
	public class MovieInfo extends EventDispatcher implements IMovieInfo, IDestroy {
		...
		private function parse() : void {
			...
			var _loc1_:Array = this._infoJSON.stl.stl as Array;
			var _loc2_:String = this._infoJSON.stl.d.toString();
			var _loc3_:Object = null;
			for each(_loc3_ in _loc1_) {
				_loc4_ = new Language();
				_loc4_.isDefault = int(_loc3_.pre) == 1;
				_loc4_.url = _loc2_ + _loc3_.l.toString();
				_loc4_.lang = Utility.getItemById(LanguageEnum.ITEMS,int(_loc3_.lid));
				this._subtitles.push(_loc4_);
				if(_loc4_.isDefault) {
					this._defaultSubtitle = _loc4_;
				}
			}
			this._focusTips = new Vector.<FocusTip>();
			_loc1_ = this._infoJSON.fl as Array;
			for each(_loc3_ in _loc1_) {
				_loc5_ = new FocusTip();
				_loc5_.index = this._focusTips.length;
				_loc5_.timestamp = Number(_loc3_.t.toString()) * 1000;
				_loc5_.content = _loc3_.c.toString();
				this._focusTips.push(_loc5_);
			}
